 ...
 * POST /api/pin           → { valid: true/false }  (PIN check)
 * GET  /api/slides?debug  → raw Notion schema (troubleshooting)
 */

const NOTION_TOKEN   = process.env.NOTION_TOKEN;
const NOTION_DB_ID   = process.env.NOTION_DB_ID;
const PRESENTER_PIN  = process.env.PRESENTER_PIN;   // ← stored in Vercel env vars
...
module.exports = async function handler(req, res) {
  ...
  if (path === '/api/pin' && req.method === 'POST') {
    const { pin } = req.body ?? {};
    if (!PRESENTER_PIN) {
      // Fallback: if env var not set, accept '2408' so nothing breaks mid-session
      return res.status(200).json({ valid: pin === '2408' });
    }
    return res.status(200).json({ valid: pin === PRESENTER_PIN });
  }
  ...
  try {
    if (req.method === 'GET' && req.query?.debug !== undefined) return await debugNotion(res);
...
// ── PIN check doesn't need Notion so it runs before the token check above ──

// ── DEBUG ─────────────────────────────────────────────────────────────────
async function debugNotion(res) {
  const dbRes  = await fetch(`https://api.notion.com/v1/databases/${NOTION_DB_ID}`, { method: 'GET', headers: nHeaders() });
  const dbData = JSON.parse(await dbRes.text());
  const rowRes  = await fetch(`https://api.notion.com/v1/databases/${NOTION_DB_ID}/query`, { method: 'POST', headers: nHeaders(), body: JSON.stringify({ page_size: 3 }) });
  const rowData = JSON.parse(await rowRes.text());
  return res.status(200).json({
    db_status:  dbRes.status,
    row_status: rowRes.status,
    properties: dbData.properties
      ? Object.entries(dbData.properties).map(([name, val]) => ({ name, type: val.type, id: val.id }))
      : dbData,
    first_rows: rowData.results?.slice(0,3).map(p => p.properties) ?? rowData,
  });
}
...
async function getSlides(res) {
  ...
  if (!r.ok) { console.error('Notion GET error:', r.status, raw); return res.status(r.status).json({ error: raw }); }
  const slides = {};
...
async function saveSlide(req, res) {
  ...
  const qr = await fetch(`https://api.notion.com/v1/databases/${NOTION_DB_ID}/query`, {
    method: 'POST', headers: nHeaders(),
    body: JSON.stringify({ filter: { property: 'slide_key', title: { equals: slide_key } } }),
  });
  const qraw = await qr.text();
  if (!qr.ok) { console.error('Query error:', qr.status, qraw); return res.status(qr.status).json({ error: qraw }); }

  const existing   = JSON.parse(qraw).results?.[0];
  const contentProp = { content: { rich_text: [{ type: 'text', text: { content: content ?? '' } }] } };
